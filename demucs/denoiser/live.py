...
def get_parser():
    parser = argparse.ArgumentParser(
        ...
        description="Performs live speech enhancement, reading audio from "
                    ...
                    "Requires a stereo input and target source in front."
        )
    parser.add_argument(
        "-i", "--in", dest="in_",
...
def main():
    ...
    print("Attempting to open input/output devices with samplerate: "+str(model.sample_rate))
    ...
    while True:
        try:
            ...
            frame = torch.permute(frame,(1,0))
            with torch.no_grad():
                out = streamer.feed(frame)[0]
                #out = frame #straight input to output
            if not out.numel():
                continue
            if args.compressor:
